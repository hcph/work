...
#**************************************************************************************************************
		...
		foreach my$chr(@CHR)
		{
			...
			print GATK "/hwfssz1/ST_MCHRI/REHEAL/USER/chendayang/00.rawdata/ifs1/bin/PGD/soft/jre1.8.0_121/bin/java -Xmx6g -jar $GATK -l INFO -T RealignerTargetCreator -R $Ref  -I $Out/chr/$chr.sort.bam -o $Out/chr/$chr.intervals && echo Making Inervals is finished!`date`\n";
			print GATK "echo \"Start to GATK Realigner!\" \n";
			print GATK "/hwfssz1/ST_MCHRI/REHEAL/USER/chendayang/00.rawdata/ifs1/bin/PGD/soft/jre1.8.0_121/bin/java -Xmx6g -jar $GATK -l INFO -T IndelRealigner -R $Ref  -I $Out/chr/$chr.sort.bam -o $Out/chr/$chr.realign.bam -targetIntervals $Out/chr/$chr.intervals && echo $Sample $chr Realigning has finished!`date` >> $Output/Project.log\n";
...
#**************************************************************************************************************
	...
	foreach my$chr(@CHR)
		...
		print GATK "/hwfssz1/ST_MCHRI/REHEAL/USER/chendayang/00.rawdata/ifs1/bin/PGD/soft/jre1.8.0_121/bin/java -Xmx6g -jar $GATK -l INFO -T UnifiedGenotyper -R $Ref  --dbsnp $dbSNP  -o $Output/chr/$chr.vcf -glm BOTH";
