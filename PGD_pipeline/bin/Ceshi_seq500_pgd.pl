...
**************************************************************************************************************************
	...
	Version:      Ver 3.0
	Date:         2017-02-16
    EarlyModifier:Li jinliang;Huang yile
	...
	Usage:        perl $0 [options]
			...
			-Target <Dir>   Directory of Target File,must content capture and gene region bed file;
					[Nemblegen: /ifs1/ST_REHEAL/USER/chendayang/bin/PGD/bed/PGD-Nemblegene/
					 BGI-V1:    /ifs1/ST_REHEAL/USER/chendayang/bin/PGD/bed/PGD-PGD-BGI-V1/
					 BGI-V2-ChipA:    /ifs1/ST_REHEAL/USER/chendayang/bin/PGD/bed/PGD-BGI-V2-Chip1/
					 BGI-V2-ChipB:    /ifs1/ST_REHEAL/USER/chendayang/bin/PGD/bed/PGD-BGI-V2-Chip2/]
	...
	Example:      perl $0 -Lib Lib -FQ FQ-Dir -Target Bed-Dir -Gene Disease-gene -Quasym [1|2] -Rmdup [Y|N] -Memory Int -Auto [Y|N] -Output OutDir -Queue queue
**************************************************************************************************************************
USAGE

my($Depth,@Sample,$h,$QuaSym,$Merge,$Lib,$FQ,$Tar,$Gene,$Memory,$Rmdup,$Output,$Auto,$queue);
GetOptions(
	'Help'    => \$h,
	'QuaSym=i'=> \$QuaSym,
	'Lib=s'   => \$Lib,
...
my $Ref     = '/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/database/GATK/ucsc.hg19.fasta';
my $bin     = '/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/bin/New_Pipeline';
my $soft    = '/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft';
my $samtools= '/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft/samtools-1.3.1/samtools';
my $samtool = '/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft/samtool';
my $enrich  = '/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft/enrichstat';
my $cov     = '/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft/soap.coverage';
my $BWA     = '/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft/bwa/bwa';
my $GATK    = '/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft/GenomeAnalysisTK.jar';
my $picard  = '/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft/picard';
my $dbSNP   = '/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/database/GATK/dbsnp_138.hg19.vcf';
my $HapMap  = '/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/database/GATK/hapmap_3.3.hg19.sites.vcf';
my $Omni    = '/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/database/GATK/1000G_omni2.5.hg19.sites.vcf';
my $Mills   = '/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/database/GATK/Mills_and_1000G_gold_standard.indels.hg19.sites.vcf';
my $Nregion = '/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/database/hg19/N_region.txt';
	...
	foreach my$num(@num){
        if($QuaSym == 1){
         ...
         #      print OUT "\n";
                ...
                print OUT "$samtools sort   $Out/temp_$_.bam > $Out/temp_$_.bam.sort.bam && echo samtools sort is completed!`date`\n";
                print OUT "/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft/jre1.8.0_121/bin/java -jar $picard/SortSam.jar I=$Out/temp_$_.bam.sort.bam  O=$Out/temp_$_.bam.sort.picard.bam SO=coordinate VALIDATION_STRINGENCY=SILENT\n";
                ...
                print OUT "/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft/jre1.8.0_121/bin/java -jar $picard/MarkDuplicates.jar INPUT=$Out/${Sample}_$_\.sort.header.bam OUTPUT=$Out/${Sample}_$_\.sort.bam METRICS_FILE=$Out/${Sample}_$_\.sort.dedup.metrics VALIDATION_STRINGENCY=SILENT  PROGRAM_RECORD_ID=MarkDuplicates PROGRAM_GROUP_NAME=MarkDuplicates\n";
#                print OUT "$samtools rmdup $Out/temp_$_.bam.sort.bam $Out/${Sample}_$_\.sort.bam && echo samtools rmdup is completed!`date`\n" if($Rmdup=~/Y/i);
                ...
                print OUT "/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft/jre1.8.0_121/bin/java -Xmx5g -Djava.io.tmpdir=/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/bin/javatmp -jar $picard/MergeSamFiles.jar\t";
...
#**************************************************************************************************************
		...
		foreach my$chr(@CHR)
		{
			...
			print GATK "/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft/jre1.8.0_121/bin/java -Xmx6g -jar $GATK -l INFO -T RealignerTargetCreator -R $Ref -L $ChrTar -I $Out/chr/$chr.sort.bam -o $Out/chr/$chr.intervals && echo Making Inervals is finished!`date`\n";
			print GATK "echo \"Start to GATK Realigner!\" \n";
			print GATK "/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft/jre1.8.0_121/bin/java -Xmx6g -jar $GATK -l INFO -T IndelRealigner -R $Ref -L $ChrTar -I $Out/chr/$chr.sort.bam -o $Out/chr/$chr.realign.bam -targetIntervals $Out/chr/$chr.intervals && echo $Sample $chr Realigning has finished!`date` >> $Output/Project.log\n";
			...
			#`/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft/jre1.8.0_121/bin/java -Xmx6g -jar /ifs2/S2PH/PGS-MD/soft/GenomeAnalysisTK.jar -l INFO -T CountCovariates -nt 8 -R $Ref1 -knownSites $dbSNP -cov ReadGroupCovariate -cov QualityScoreCovariate -cov CycleCovariate -cov DinucCovariate -I $out\/temp\.realign\.bam -recalFile $out\/temp\.recal`;
			#`/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft/jre1.8.0_121/bin/java -Xmx6g -jar /ifs2/S2PH/PGS-MD/soft/GenomeAnalysisTK.jar -l INFO -R $Ref1 -noOQs -T TableRecalibration -I $out\/temp\.realign\.bam -recalFile $out\/temp\.recal -o $out\/temp\.realign\.recal\.bam`;
...
#**************************************************************************************************************
	...
	foreach my$chr(@CHR)
		...
		#print GATK "/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft/jre1.8.0_121/bin/java -Xmx6g -jar $GATK -l INFO -T HaplotypeCaller -R $Ref -L $ChrTar --dbsnp $dbSNP -stand_call_conf 20 -stand_emit_conf 4 -ip 200 -o $Output/chr/$chr.vcf -A IndelType -A QualByDepth -A HaplotypeScore -A MappingQualityRankSumTest -A ReadPosRankSumTest -A FisherStrand -A DepthOfCoverage";
		print GATK "/ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft/jre1.8.0_121/bin/java -Xmx6g -jar $GATK -l INFO -T UnifiedGenotyper -R $Ref -L $ChrTar --dbsnp $dbSNP  -o $Output/chr/$chr.vcf -glm BOTH";
...
#**************************************************************************************************************
	...
	print ANNO "perl $soft/summarize_annovar.pl --remove --buildver hg19 --verdbsnp 138 $Output/anno/Family.SNP.INDEL.anno /ifs1/ST_REHEAL/USER/chendayang/bin/PGD/soft/humandb  && echo Mutation annovar if finished!`date`\n";
	...
	if(defined $Gene){print ANNO "perl $bin/PGD_MD_summary_report.pl -r $Tar/target.gene -l $Output/$Lib -a $Output/anno/Exome_mutation.csv -g $Gene -s $Output/Hap/Family.score.xls -o $Output/PGD-MD-Project-case-report.xls && echo Congratelatopns, all progresses were done at `date`! >> $Output/Project.log\n";}
...
if($Auto=~/Y/){
	...
	map{$record=`cd $Output/sh/ && qsub -cwd -P $queue -l p=5  -l  num_proc=10   -l vf=1g step1-$_-Alignment.sh`;push @job,(split /\s+/,$record)[2]} keys %Sample;
	$job_aln=join ",",@job;@job=(); 
	map{$record=`cd $Output/sh/ && qsub -cwd -P $queue -l p=5  -l  num_proc=10  -l vf=5g -hold_jid $job_aln step2-$_-Realignment.sh`;push @job,(split /\s+/,$record)[2]} keys %Sample;
	$job_realn=join ",",@job;@job=(); 
	map{$record=`cd $Output/sh/ && qsub -cwd -P $queue -l p=5 -l  num_proc=10  -l vf=3g -hold_jid $job_realn step3-Genotyper-chr$_.sh`;push @job,(split /\s+/,$record)[2]}(1..22,'X','Y');
	$job_gt=join ",",@job;@job=(); 
	$record=`cd $Output/sh/ && qsub -cwd -l vf=3g -P $queue  -l  p=5  -l  num_proc=10  -hold_jid $job_gt step4-Correct.sh`;
	push @job , (split /\s+/,$record)[2];
	$job_var=join ",",@job;
	$record=`cd $Output/sh/ && qsub -cwd -l vf=5g -P $queue  -l   p=5  -l  num_proc=10  -m abes -M chendayang\@genomics.cn  -hold_jid $job_var step5-Anno.sh`;
}
